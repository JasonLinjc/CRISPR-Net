...
offtarget_df = pd.read_csv("./input_examples/gecko_GACCTTGCATTGTACCCGAG_mix_gene.csv")
gRNA_seq = offtarget_df['crRNA']
offtarget_seq = offtarget_df['DNA']
genic = offtarget_df['Gene_mark']
# print(genic)
genic_labels = np.ones(len(genic))
genic_labels[genic.isnull()] = 0
# print(genic_labels)
run_CRISPR_net_aggregate(gRNA_seq, offtarget_seq, genic_labels)
